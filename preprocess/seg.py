...
if __name__ == "__main__":
    ...
    dataset_path = "/home/ibrahim.almakky/datasets/CT/"

    train_file = "./dataset/train_hust.csv"
    val_file = "./dataset/val_hust.csv"
    test_file = "./dataset/test_hust.csv"

    save_path = "./dataset/hust-seg/"
    ...
    def seg_dicom(train_file):
        ...
        save_txt_file = train_file.replace(".csv", "-seg.text")
        segimgfiles = []
        for index, file_dir in enumerate(file_dirs):
            file_dir = os.path.join(dataset_path, file_dir)
            dcm_files = glob(file_dir)
            print(dcm_files)
            # make sure npy save path
            if labels[index] == 1:
                npy_save_path = file_dir.replace(dataset_path, save_path)
                target_path = npy_save_path.split("/*")[0]
                segimg_save_path = npy_save_path.replace(IMG_EXT, "_segimg.npy")
                img_save_path = npy_save_path.replace(IMG_EXT, "_img.npy")
                mask_save_path = npy_save_path.replace(IMG_EXT, "_mask.npy")
